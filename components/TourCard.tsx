...
export default function TourCard({
  ...
  return (
    <article className="flex flex-col rounded-card bg-white p-4 sm:p-5">
      <span className="chip chip-muted-brand w-fit">{tour.region}</span>
      ...
      <div className="relative mt-4 aspect-[4/3] overflow-hidden rounded-card-inner">
        ...
        <div className="absolute inset-x-0 bottom-0 px-4 pb-4">
          <p className="text-caption font-medium text-white">
            {tour.hotel}{" "}
            <span className="text-white/90">{"★".repeat(tour.rating)}</span>
          </p>
        </div>
      </div>
      ...
      <div className="mt-4 space-y-3">
        <InfoRow
          label={tour.dateRange}
          value={`${tour.nights} ночей`}
          withDivider={false}
        />
        <InfoRow label="Харчування" value={tour.meals} />
        <div className="divider pt-3">
          <div className="flex items-end justify-between gap-3">
            <span className="text-caption max-w-[120px] leading-snug text-subtitle">
              Ціна від / за 2 дорослих
            </span>
